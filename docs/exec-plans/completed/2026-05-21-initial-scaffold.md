...
Scaffolded `ai-meeting-notes` from the `vibe-coding-starter-kit` template
...
- Identifier rename `vibe-coding-starter-kit` -> `ai-meeting-notes`
  across every text file in the tree (excluding `node_modules`, `.next`,
  `dist`, `build`, `.venv`).
- Workspace packages: `@ai-meeting-notes/{shared,web}`.
- Custom user agent set on the only S3 client to
  `b2ai-ai-meeting-notes`.
