...
`<MeetingCard />` (in `components/meetings/meeting-card.tsx`) is the
default primitive for the Meetings grid. It renders any `Meeting` with:

- A two-line filename / title link (clickable -> detail page)
- A pipeline state badge (Queued / Transcribing / Summarizing / Done / Failed) with state-tinted background
- A one-line summary excerpt when the LLM stage has completed; a placeholder otherwise
- Open and Delete actions; Delete is gated by an `AlertDialog`

### When to use
Default for any list of meetings under the `meetings/` prefix. Compose
into a grid (`sm:grid-cols-2 xl:grid-cols-3`) like `apps/web/src/app/meetings/page.tsx`
does.

### Transcript primitives
- `<TranscriptView />` — speaker-turn list with click-to-seek + current-segment highlight. Takes the `Transcript` object and a `currentTimeMs` prop driven by the `<audio>` element's `onTimeUpdate`.
- `<SummaryPanel />` — summary text, decisions list, topic chips.
- `<ActionItemsList />` — checkbox list with optional owner / due and a "Source" deep-link back to the transcript via `onSeekSegment`.
- `<PipelineStatus />` — compact per-stage indicator that hides itself once the pipeline reaches `done`.
- `<Waveform />` — lightweight SVG waveform stub re-homed from the starter; kept for the meeting playback surface.

### Composing your own meeting view
1. Add a query hook to `lib/queries.ts` (mirror `useMeeting` / `useMeetingTranscript`).
2. Make sure your data serializes to the `Meeting` / `Transcript` / `Summary` / `ActionsBundle` shapes from `packages/shared/src/types.ts`.
3. Reuse `<MeetingCard />`, `<TranscriptView />`, `<SummaryPanel />`, and `<ActionItemsList />` verbatim — all are net-new (non-shadcn) primitives and safe to depend on directly.

The live showcase lives at `/design` under "Meeting Card".
