...
function checkEnv() {
  ...
  // Provider keys — warn only. The bucket + upload UI works without
  // them; the pipeline simply marks each meeting `failed` on the
  // missing-key error.
  if (!env.ASSEMBLYAI_API_KEY) {
    warn(
      "ASSEMBLYAI_API_KEY is not set — transcription will fail",
      "Add ASSEMBLYAI_API_KEY to .env (https://www.assemblyai.com/app/account)",
    );
  }
  const llmProvider = (env.LLM_PROVIDER || "openai").toLowerCase();
  if (llmProvider === "anthropic" && !env.ANTHROPIC_API_KEY) {
    warn(
      "LLM_PROVIDER=anthropic but ANTHROPIC_API_KEY is not set — summary + action-item extraction will fail",
      "Add ANTHROPIC_API_KEY to .env (https://console.anthropic.com/) or switch LLM_PROVIDER to openai",
    );
  } else if (llmProvider !== "anthropic" && !env.OPENAI_API_KEY) {
    warn(
      "OPENAI_API_KEY is not set — summary + action-item extraction will fail",
      "Add OPENAI_API_KEY to .env (https://platform.openai.com/api-keys) or switch LLM_PROVIDER to anthropic",
    );
  }
}
