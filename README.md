...
1. **Create a bucket.** B2 will show the bucket name and endpoint details. Paste these into `.env`:
   - **Bucket Unique Name** -> `B2_BUCKET_NAME`
   - **Region** (the path segment of the endpoint, e.g. `us-west-004`) -> `B2_REGION`
   - The S3 endpoint is derived from `B2_REGION`; do not add an endpoint alias.
2. **Create an application key** with `Read and Write` permission. Paste into `.env`:
   - **keyID** -> `B2_APPLICATION_KEY_ID`
   - **applicationKey** -> `B2_APPLICATION_KEY` *(only shown once — paste it now)*
3. **Optionally set a public URL base** with `B2_PUBLIC_URL_BASE` if the bucket is public or fronted by a CDN.
