...
### Test types
- **Unit**: pure logic (service layer — meeting id validation, search snippet extraction)
- **Integration**: HTTP handlers, B2 connectivity (`tests/`)
- **Structural**: layering rules, import boundaries, SDK containment (`tests/test_structure.py`)
- **E2E**: Playwright browser-driven smoke tests

### Adding an LLM provider
The LLM adapter pattern lives in `services/api/app/repo/llm.py`. It exposes a single `_structured_call(prompt, name, schema, description)` that runs whichever provider `LLM_PROVIDER` selects. To add a new provider:
1. Add a `_<name>_client()` factory using the lazy `@functools.lru_cache(maxsize=1)` + env-var pattern; raise `LLMError` if the key is missing.
2. Add a `_call_<name>(prompt, name, schema)` helper that calls the SDK with the provider's structured-output mechanism and returns a dict.
3. Add a branch in `_structured_call()` keyed off `LLM_PROVIDER`.
4. Make `_model()` aware of the provider's `<NAME>_MODEL` env var + default.
5. Document the env var(s) in `.env.example` and `scripts/doctor.mjs`.
6. List the SDK in `requirements.txt`.
The service layer is provider-agnostic — `service/pipeline.py` only calls `summarize()` / `extract_actions()` and consumes the returned dict. The same applies to transcription, though only AssemblyAI is wired today.

### Adding a pipeline stage
The orchestrator in `services/api/app/service/pipeline.py` is a linear `_run_<stage>` chain. To add a stage:
1. Write the artifact's Pydantic model in `app/types/meeting.py`.
2. Add a writer + (optional) parallel-reader in `app/repo/meetings_store.py`.
3. Implement the stage logic in `app/repo/<provider>.py` (provider adapter) or a new `service/` module if it's pure business logic.
4. Insert a `_run_<stage>` helper in `service/pipeline.py` that rewrites `status.json` before doing its work.
5. Wire it into `run_pipeline()` in the canonical order.
...
- Doctor env contract tests: `pnpm test:doctor`
